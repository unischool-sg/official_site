import { unauthorizedResponse, serverErrorResponse, validationErrorResponse, successResponse } from "@/lib/api/response";
...
export async function POST(req: NextRequest) {
    try {
        const [formData, user] = await Promise.all([req.formData(), User.current()]);
        if (!user) {
            return unauthorizedResponse("認証が必要です");
        }
        ...
        if (!avatarFile) {
            return validationErrorResponse(["アバター画像が提供されていません"]);
        }
        ...
        // アバター画像をアップロード
        const response = await fetch(`https://storage.evex.land/upload?filename=${user.id}`, {
            method: "POST",
            body: arrayBuffer,
            headers: {
        ...
        if (!response.ok) {
            return serverErrorResponse("アバター画像のアップロードに失敗しました");
        }
        ...
        return serverErrorResponse("アバター画像のアップロード中にエラーが発生しました");
    }
}