"use server";
import { createdResponse, unauthorizedResponse, validationErrorResponse } from "@/lib/api/response";
...
export async function POST(req: NextRequest) {
    const authToken = await req.headers.get("authorization")?.replace("Bearer ", "");
    if (!authToken) {
        return unauthorizedResponse("トークンが提供されていません");
    }
    const token = await Token.get(authToken);
    console.log(token);
    if (!token || token.expires < new Date() || token.type !== "REGISTRATION_CONFIRMATION") {
        return unauthorizedResponse("無効なトークンです");
    }
    const [user, data] = await Promise.all([token.user, req.json()]);
    if (!user) {
        return unauthorizedResponse("トークンに関連付けられたユーザーが見つかりません");
    }

    const { name, bio, password }: { [key: string]: string } = data;
    if (!name || !password) {
        return validationErrorResponse(["名前は必須です", "パスワードは必須です"]);
    }
    ...
    return createdResponse({ user, message: "アカウントが作成され、ログインしました" });
}