...
async function handleRegistTokenUser(e: React.FormEvent<HTMLFormElement>, setIsLoading: React.Dispatch<React.SetStateAction<boolean>>, setError: React.Dispatch<React.SetStateAction<string | null>>, router: AppRouterInstance) {
    e.preventDefault();
    ...
    try {
        ...
        toast.success("ユーザーが登録されました", { description: data.data.user.email });
        form.reset();
    } catch (error) {
        console.error("Error registering user:", error);
        toast.error("ユーザーの登録に失敗しました", { description: (error as Error).message });
    } finally {
        setIsLoading(false);
    }
