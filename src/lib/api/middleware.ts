 ...
 * @returns ラップされたハンドラー
 */
export function withRole(handler: ApiHandler, allowedRoles: UserRole[]) {
     return async (
          request: NextRequest,
          routeContext?: RouteContext,
     ): Promise<Response> => {
...
export function withErrorHandler(handler: ApiHandler) {
     return async (
          ...
          try {
               ...
               // コンテキストを作成（ユーザーなし）
               const context: RequestContext | undefined = params
                    ? { user: null as any, params }
                    : undefined;
...
export function getPaginationParams(request: NextRequest): {
     ...
     const limit = Math.min(
          100,
          Math.max(1, getNumberParam(request, "limit", 10) || 10),
     );
